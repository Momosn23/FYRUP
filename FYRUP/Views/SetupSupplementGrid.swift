        ...
        LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 10), count: typeSize.isAccessibilitySize ? 2 : 3), spacing: 10) {
            ForEach(names, id: \.self) { name in
                ...
                Button {
                    ...
                    VStack(spacing: 8) {
                        Image(systemName: name == "Vitamin D3" ? "sun.max" : name == "Eigener Eintrag" ? "plus" : "pills")
                            .font(.title2).accessibilityHidden(true)
                        Text(name).font(.footnote.weight(.medium)).multilineTextAlignment(.center).fixedSize(horizontal: false, vertical: true)
                        Image(systemName: saved == nil ? "circle" : "checkmark.circle.fill").foregroundStyle(saved == nil ? FYColor.line : FYColor.lime)
                    }.frame(maxWidth: .infinity, minHeight: 94).padding(8)
