...
final class SupplementStore {
    ...
    init(repository: any SupplementRepository, persistence: (any SupplementPendingPersistence)? = nil) {
    ...
    func activate(userID: UUID?) {
        ...
        errorMessage = nil; pendingError = nil
        guard let userID else { return }
        do {
            guard let data = try persistence.load(owner: userID) else { return }
            ...
            storageBlocked = true
            pendingError = "Lokale Bestätigungen konnten nicht gelesen werden. Sie bleiben erhalten; neue Bestätigungen sind bis zum erneuten Laden gesperrt."
        }
    }
    ...
    func save(_ value: SupplementPlan) async -> Bool {
        ...
        do {
            ...
            self.snapshot = next; errorMessage = nil; return true
        } catch {
    ...
    func mark(_ doseID: UUID, as status: SupplementDoseStatus) async {
        guard !storageBlocked, !isSaving, !isSyncing, let snapshot,
        ...
        let change = SupplementDoseMutation(doseID: dose.id, status: status, expectedRevision: dose.revision)
