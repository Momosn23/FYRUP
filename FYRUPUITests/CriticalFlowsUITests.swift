...
final class CriticalFlowsUITests: XCTestCase {
    ...
    func testDiscoverPreservesTheChosenSport() {
        ...
        revealAndTap(app.buttons["discover-filter-sports"], in: app)
    ...
    func testProfilePrivacyAndLogoutFlow() {
        ...
        revealAndTap(app.buttons["Privatsphäre"], in: app)
        ...
        app.navigationBars.buttons.element(boundBy: 0).tap()
        revealAndTap(app.buttons["Abmelden"], in: app)
    ...
    func testSettingsDestinationsWork() {
        ...
        XCTAssertTrue(app.staticTexts["Hilfe & FYRUP"].waitForExistence(timeout: 3))
    ...
    func testNotificationsReferenceScreen() {
        ...
        XCTAssertTrue(app.staticTexts["Mitteilungen"].waitForExistence(timeout: 2))
