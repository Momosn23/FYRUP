...
- Neue Migration `202609050015_activity_place.sql` ergänzt den auf 120 Zeichen begrenzten Ortsnamen, übernimmt vorhandene Session-Treffpunkte und stellt kompatible neue RPC-Signaturen bereit. Alte App-Versionen behalten ihre bisherigen Signaturen. Die Migration wurde am 05.09.2026 über den atomaren, vorab getesteten Deployment-Block produktiv angewendet. Eine frische Abfrage in einem getrennten SQL-Tab bestätigte Verlaufseintrag, Spalte, Längen-Constraint, authentifizierte Rechte für beide neue RPCs und die anonyme Sperre. Es wurden keine Testnutzer oder Beispielaktivitäten erzeugt.
...
- Ein Demo-Lebenszyklustest prüft Kürzung und Rückgabe des Orts; ein neuer Bedienablauf erfasst den spontanen Ort. Der lokale Wegwerf-Datenbanktest prüft die atomare Migration, Rücksetzung bei Fehler, Wiederholungssperre, Rechte, Kürzung und 120-Zeichen-Grenze. Sprachprüfung und lokale Release-Prüfung sind grün; die produktive Migration ist verifiziert. Native Swift-/UI-Ausführung und visuelle Abnahme bleiben offen.
...
- Zwei Speicher-/Rückkehrtests sind vorbereitet. Die Apple-App-Gruppe `group.app.fyrup.shared` ist registriert und den Kennungen `app.fyrup.ios` sowie `app.fyrup.ios.live` zugeordnet. Die Profile „FYRUP App Store 2026“ und „FYRUP Live App Store 2026“ wurden danach am 05.09.2026 neu erzeugt, sind bei Apple wieder gültig und als `fyrup_app_store_appgroup` sowie `fyrup_live_app_store_appgroup` in Codemagic gespeichert. Der Workflow verweist auf diese neuen Namen; alte Profile wurden nicht gelöscht. Für eine echte Auslieferung fehlen noch ein nativer Swift-6-Lauf und die Prüfung auf einem iPhone.
...
`a964fe6` mit dieser ersten Korrektur wurde anschließend auf `main` geladen. [Codemagic 39](https://codemagic.io/app/6a9aff9f64377f6028cc8d18/build/6a9c75b03cf4759eab7e97fe) bestätigte, dass der Widget-Fehler passiert wird, und stoppte nach 48 Sekunden im Hauptziel bei `SessionIntervalView.swift:72`: Ein `@State`-Wrapper war auf die drei Intervallfelder `work`, `recovery` und `rounds` gemeinsam angewendet. Diese unter Swift 6 unzulässige Deklaration ist für den nächsten Lauf in drei einzelne Zustände getrennt. Tests, Bildschirmbilder und Artefakte wurden auch in Lauf 39 noch nicht erreicht.

`e0e1192` mit der zweiten Korrektur wurde danach auf `main` geladen. [Codemagic 40](https://codemagic.io/app/6a9aff9f64377f6028cc8d18/build/6a9c7715fac9a246beda8cc5) kam über beide vorherigen Compilerstellen hinaus und stoppte nach 2 Minuten 6 Sekunden beim Übersetzen von `DemoRepository.swift:156/161` und `DemoWorkoutRepository.swift:290`: Der zuvor dateiprivate Leerzeichen-Helfer `trimmedNil` wird durch die neue freiwillige Ortsangabe nun außerhalb von `ActivityViews.swift` benötigt. Er ist im Folgequellstand app-intern statt dateiprivat; dadurch entsteht keine öffentliche API. Native Tests und Bildschirmbilder wurden in Lauf 40 noch nicht erreicht.

`91899a7` mit der dritten Korrektur wurde anschließend auf `main` geladen. [Codemagic 41](https://codemagic.io/app/6a9aff9f64377f6028cc8d18/build/6a9c7875fac9a246beda8fb0) übersetzte App und Widget vollständig und gelangte bis zum Testziel. Der Lauf stoppte nach 1 Minute 53 Sekunden beim Übersetzen von `ActiveCalorieEstimateTests.swift:46`: Der Isolationstest wollte `Activity.userID` nach der Erstellung ändern, obwohl die Eigentümer-ID absichtlich unveränderlich ist. Im Folgequellstand nimmt die Testhilfe eine optionale Nutzer-ID entgegen und erzeugt die Fremdaktivität von Anfang an korrekt; das Produktionsmodell bleibt unverändert.

`aa243a2` mit dieser vierten Korrektur wurde danach auf `main` geladen. [Codemagic 42](https://codemagic.io/app/6a9aff9f64377f6028cc8d18/build/6a9c79a3fac9a246beda923e) übersetzte App, Widget, Swift-Einzeltests und UI-Testziel vollständig. 458 von 459 Swift-Einzeltests bestanden. Nur `ArrivalReminderTests.testReminderIsAccountIsolatedAndRemovedWhenSessionChanges` prüfte den asynchron gestarteten Systemhinweis-Abgleich unmittelbar und damit gelegentlich vor dessen Ausführung; die Produktdaten waren zu diesem Zeitpunkt bereits korrekt entfernt. Der Folgequellstand wartet in diesem Test mit einer festen Zwei-Sekunden-Obergrenze auf den beobachtbaren Aufräumaufruf. Der begonnene UI-Testlauf wurde nach dem roten Einzeltest abgebrochen; neue Bildschirmbilder sind deshalb noch kein vollständiger visueller Nachweis.
# Fortsetzung nach Codemagic 43

- `20c52bc` ist auf `main`; Codemagic 43 übersetzte App, Live-Aktivität/Widget und Testziele vollständig.
- Der zuvor einzige Einzeltestfehler aus Build 42 ist behoben; der Lauf erreichte alle 33 UI-Abläufe.
- Ergebnis UI: 27 bestanden, 6 fehlgeschlagen. Betroffen waren Blind-Workout-Satzpause, Onboarding-Abschluss, eigene Satzpausendauer, Stammgym-Ort ändern/leeren, Satzpause auf Heute/LIVE und Laufintervalle.
- Gemeinsame Ursache der vier Timerfehler: Die Kennung der äußeren SwiftUI-Karte überschrieb die Kennungen der inneren Schaltflächen. Die Eltern-Kennungen sind lokal entfernt; die konkreten Aktionen behalten nun ihre eigenen zugänglichen Kennungen.
- Onboarding verwendet lokal einen eigenen persönlichen Einrichtungsweg und wechselt nach erfolgreichem Abschluss ausdrücklich zur Abschlussseite.
- Sofortiger und geplanter Ort besitzen lokal eine sichtbare Leeren-Aktion; der Stammgym-UI-Test nutzt denselben echten Bedienweg.
- Diese lokalen Korrekturen sind noch nicht nativ nachgeprüft oder ausgeliefert. Erst nach den lokalen statischen Prüfungen wird ein einzelner gebündelter Folgebuild gestartet.
