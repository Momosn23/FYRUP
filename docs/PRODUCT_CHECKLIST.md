...
Aktueller Folgelauf: Codemagic 54 (`7688aaa`) bestätigte die Korrektur des Willkommensablaufs; 36 von 37 Bedienabläufen bestanden. Der Lauf startete jedoch um 01:26 CEST so nah am Tageswechsel, dass die standardmäßig eine Stunde später angesetzte Test-Session tatsächlich auf den Folgetag fiel. Zwei Einzeltests und ein Bedienablauf erwarteten sie trotzdem fälschlich in „Heute“. Der lokale Folgequellstand prüft die angelegte Session unabhängig vom Kalendertag, aktualisiert nach dem Planen die Wochenübersicht sofort und macht geplante eigene Sessions darin direkt über „SESSION ÖFFNEN“ erreichbar. Diese Korrektur ist lokal geprüft (`git diff --check` und Sprachprüfung), aber noch nicht nativ abgenommen. Migration 016 für den privaten Übungsverlauf ist produktiv angewendet und unabhängig geprüft: Funktion vorhanden, Ausführung für `authenticated` erlaubt sowie für `anon` und `public` gesperrt.
