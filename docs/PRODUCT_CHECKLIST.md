...
Aktueller Folgelauf: [Codemagic 55](https://codemagic.io/app/6a9aff9f64377f6028cc8d18/build/6a9e0312fac9a246beddbe15) prüfte Commit `6d0fd1b` vollständig: **474 von 474 Einzeltests und 37 von 37 Bedienabläufen bestanden; `TEST SUCCEEDED`.** Damit sind die tageswechsel-feste Session-Prüfung, die sofort aktualisierte Wochenübersicht sowie „SESSION ÖFFNEN“ für eigene geplante Sessions nativ im Simulator bestätigt. Der einmalig gestartete [signierte TestFlight-Folgelauf](https://codemagic.io/app/6a9aff9f64377f6028cc8d18/build/6a9e12c7daadaa98524a899f) stoppte vor der IPA-Erstellung: Der Release-Testbefehl hatte `ENABLE_TESTABILITY` nicht gesetzt, weshalb die Tests das korrekt erzeugte App-Modul nicht per `@testable import` laden konnten. Der Folgequellstand setzt die Option nur für diesen Testbefehl, nicht für das spätere Store-Archiv. Eine neue IPA oder Apple-Verarbeitung wird vor einem erfolgreichen Folgelauf nicht behauptet. Migration 016 für den privaten Übungsverlauf ist produktiv angewendet und unabhängig geprüft: Funktion vorhanden, Ausführung für `authenticated` erlaubt sowie für `anon` und `public` gesperrt.
...
- [x] Die bisher wirkungslosen Einstellungszeilen „Freunde“, „Hilfe & Support“ und „Über FYRUP“ haben echte Ziele; Support bereitet eine Nachricht an `Kundenservice@objektsignal.com` vor und versendet nie automatisch. Der gemeinsame Bedienablauf bestand in Codemagic 55; die installierbare TestFlight-Abnahme bleibt bis zum signierten Folgelauf offen.
