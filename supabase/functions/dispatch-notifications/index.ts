...
import { dispatchQueuedNotification, notificationPayload, notificationDeliveryHeaders } from "./dispatch-queue.mjs";
...
Deno.serve(async (request) => {
  ...
  const scheduled = await db.rpc("process_supplement_reminders");
  if (scheduled.error || !Number.isInteger(scheduled.data) || scheduled.data < 0) return json({ error: "Reminder scheduling unavailable" }, 500);
  ...
  for (const note of notifications) {
    ...
    const result = await dispatchQueuedNotification({ db, note, prepareToSend: async () => {
      ...
      return await fetch(`https://${host}/3/device/${device.token}`, {
        method: "POST",
        headers: { authorization: `bearer ${token}`, "apns-topic": topic, "apns-push-type": "alert", "apns-priority": "10", ...notificationDeliveryHeaders(authorizedNote) },
