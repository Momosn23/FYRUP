// Disposable PostgreSQL WASM verification; no network, secrets or production DB.
// Uses pinned .qa/workout-db dependencies from run_workout_tests.mjs.
// node supabase/tests/run_weekly_tests.mjs
...
try {
  ...
  // Legacy data precedes the new migration, exercising conservative backfill.
  ...
  await db.exec(await readFile(resolve('supabase/migrations/202609050005_weekly_flames.sql'), 'utf8'));
  console.log('Applied 202609050005_weekly_flames.sql over legacy fixtures');
  for (const filename of ['weekly_flames.sql', 'workout_plans.sql', 'activity_pause.sql', 'daily_steps.sql']) {
    ...
    assert.ok(!tap.some(line => /(^|\n)not ok |(^|\n)#.*(?:failed|planned .* but ran)/i.test(line)), 'Weekly database tests failed');
  }
  console.log('PASS: weekly migration/backfill/RPC/RLS/timezone suite and prior workout/pause/steps regression suites. Physical concurrent sessions, hosted Supabase/PostgREST, push delivery and native iOS NOT tested.');
} catch (error) {
